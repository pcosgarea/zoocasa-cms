...
This document outlines the "spec" for the Zoocasa CMS along with the Claude Code plan file, [`plan.md`](./plan.md).

A summary of all work completed so far is stored in [`claude-code-checkpoint.md`](./claude-code-checkpoint.md), with a higher-level summary also in [`progress.md`](./progress.md) (see instructions below on resuming development with Claude Code).
